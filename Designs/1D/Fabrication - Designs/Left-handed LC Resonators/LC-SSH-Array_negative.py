...
def ghost_feedline_negative(Q, unitcell_size,startM, strip_height, tw, tv, M, U, w=500e-9, t = 2e-6, Sg = 10e-6,T=100e-6,D=200e-6,R=50e-6,f=24e-6, r=29e-6,A = 50e-6, wf=[360e-6,90e-6,4e-6],maskdim = [150e-6,300e-6],cozy = False):
# =============================================================================
...
# =============================================================================
    ...
    w_patch = wf[0]*1e6
    w_core = wf[1]*1e6
    w_start = wf[2]*1e6#start width feedline
    chip_length = 6200
    maskdim = maskdim[0]*1e6, maskdim[1]*1e6
    d_overlap = 1 #overlap between the two precision layers
    ...
    width_feedline = [w_patch,w_core,w_start]
    width_guide = [50/45*w for w in width_feedline] #original value: 73/45
    width_guide[1] = 31/30*width_feedline[1]
    width_guide[2] = 11/5 * width_feedline[2]
    ...
    #coordinates for left feedline + guide (FlexPath)
    points2 = [[x2+E,Y0],[x2+7*E/8, Y0],[x2+3*E/4, Y0],[x2,Y0],[x4 + D, Y0],[startM[0]+A/2+Q*unitcell_size[0],Y0]]
...
if BIG == True:
    ...
    Rc = 10e-6#200e-6                         #extent of ground planes on each side of the array
    wfT = [360e-6,90e-6,10e-6]          #width feedline for T-geometry
...
if GGG == True:
    ...
    unitcell, ground, startM, startU, stopUy, strip_height, Bc, center1st, center2nd = unit_cell_upup(L,s,w,Ac,tc,tv,tw,e=ep,f=fp,r=rp,gamma=k,ground_in_between = ground_yn)
else:
    unitcell, ground, startM,startU,stopUy,strip_height, Bc, center1st, center2nd = unit_cell(L,s,w,Ac,tc,tv,tw,e=ep,f=fp,r=rp, gamma = k, ground_in_between=ground_yn)
...
if Tfeed == True:
    ...
    U_g = [Ug1, Ug2]
    M_g = [Mg1, Mg2]
    groundplane_coords,pads,mask, mask_overlap = ghost_feedline_negative(Q, unitcell_size, startM, strip_height, tw, tv, M_g, U_g,w=w,t=tc,Sg = Sgg,T=Tg,D=Df,R=Rc,f=fp,r=rp,A=Ac,wf=wfT,maskdim=[100e-6,300e-6])
    GrdArray = gdspy.CellArray(ground, Q, 1, unitcell_size)
elif ghostfeed == True and upup == True:
    U_g, M_g, Bc = OPKI_up(L,s,w,Ac,tc)
...
lib.add(design)
gdspy.LayoutViewer()
lib.write_gds("64LC-Array_normal.gds")
